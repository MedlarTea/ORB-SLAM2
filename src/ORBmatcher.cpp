...
/**
 * @brief 通过投影，对Local MapPoint进行跟踪(Local Mapping中)
 * 对于每个局部3D点通过投影在小范围内找到和最匹配的2D点
...
// 用于闭环检测中将MapPoint和关键帧的特征点进行关联(Loop Closure中)
...
/**
 * @brief 利用基本矩阵F12，在两个关键帧之间未匹配的特征点中关联新的3d点
 * 
 * @param pKF1          关键帧1
 * @param pKF2          关键帧2
 * @param F12           基础矩阵
...
/**
 * @brief 将MapPoints投影到关键帧pKF中，并判断是否有重复的MapPoints(Local Mapping中)
...
// 投影MapPoints到KeyFrame中，并判断是否有重复的MapPoints(闭环检测中)
...
// 与searchbyprojection的区别:
// 1.有旋转, 平移, 尺度信息
// 2.searchbyprojection是某帧3D点投影到另一帧，进行2D-2D的reprojection error分析;searchbysim3是两者都有3D点，相互投影，共同筛选确定
...
/**
 * @brief 通过投影，对上一帧的特征点进行跟踪(Tracking中)
 *
...
/** 当前帧与上一帧的关键帧的匹配(ReLocalization中)
 * 对当前帧每个3D点通过投影在关键帧小范围内找到最匹配的2D点，从而实现当前帧与关键帧3D点的匹配跟踪。用于重定位时的特征匹配
 * 
 */
