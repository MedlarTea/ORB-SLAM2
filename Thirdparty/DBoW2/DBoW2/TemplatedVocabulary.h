  ...
  const std::vector<std::vector<TDescriptor> > &training_features)
{
  ...
  // create the tree 分层聚类
  HKmeansStep(0, features, 1);

  // create the words 即tree的叶子结点,保存为word
  createWords();

  // and set the weight of each node of the tree
  setNodeWeights(training_features);  // TF-IDF赋权重
  ...
  const vector<pDescriptor> &descriptors, int current_level)
{
  ...
  else
  {
    ...
    while(goon)
    {
      ...
      else
      {
        ...
        for(unsigned int c = 0; c < clusters.size(); ++c)
        {
          ...
          */
          ...
          F::meanValue(cluster_descriptors, clusters[c]);  //求每一类的均值，得到新的簇
        }
      ...
      for(fit = descriptors.begin(); fit != descriptors.end(); ++fit)//, ++d)
      {
        ...
        groups[icluster].push_back(fit - descriptors.begin()); // 这样增添索引可以避免迭代器的改变使得迭代器失效的问题
  ...
  // create nodes， 将前面聚类结果存入Nodes
  for(unsigned int i = 0; i < clusters.size(); ++i)
  {
    NodeId id = m_nodes.size();
    m_nodes.push_back(Node(id));
    m_nodes.back().descriptor = clusters[i];  // 描述子
